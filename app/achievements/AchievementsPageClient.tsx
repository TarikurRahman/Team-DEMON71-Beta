          ...
          >
            ...
            <div className="p-6 md:p-8 flex flex-col flex-grow justify-between">
              <div>
                ...
                <h2 className="text-white font-extrabold text-lg md:text-xl tracking-wide group-hover:text-[#de4030] transition-colors duration-300 line-clamp-2">
                  {item.subTitle}
                </h2>
                <p className="text-zinc-400 text-xs md:text-sm mt-3 leading-relaxed text-justify line-clamp-2">
                  {item.description}
                </p>
              </div>

              <div className="mt-4 pt-4 border-t border-zinc-900/80 space-y-1.5 text-xs text-zinc-500 font-medium">
                <div className="flex items-center gap-2">
                  <Calendar className="w-3.5 h-3.5 text-[#de4030]/70 flex-shrink-0" />
                  <span>{item.date}</span>
                </div>
                <div className="flex items-center gap-2">
                  <MapPin className="w-3.5 h-3.5 text-[#de4030]/70 flex-shrink-0" />
                  <span className="truncate text-[10px]">{item.venue}</span>
                </div>
              </div>
            </div>
          </motion.div>
      ...
      <AnimatePresence>
        {selectedAchievement && (
          <div className="fixed inset-0 bg-black/85 backdrop-blur-md flex items-center justify-center z-50 p-4">
            <motion.div layoutId={`card-${selectedAchievement.id}`} className="bg-zinc-950 border border-zinc-900 rounded-3xl w-full max-w-2xl max-h-[85vh] overflow-y-auto relative shadow-2xl">
              <button onClick={() => setSelectedAchievement(null)} className="absolute top-4 right-4 z-20 p-2 rounded-full bg-black/60 border border-zinc-800 text-zinc-400 hover:text-white transition-all">
                <X className="w-4 h-4" />
              </button>

              <div className="aspect-video w-full overflow-hidden bg-zinc-900">
                <img src={selectedAchievement.image} alt={selectedAchievement.subTitle} className="object-cover w-full h-full" onError={(e) => { e.currentTarget.src = "https://images.unsplash.com/photo-1567427017947-545c5f8d16ad?auto=format&fit=crop&w=800&q=80"; }} />
              </div>

              <div className="p-8">
                <h2 className="text-2xl font-black uppercase text-[#de4030] tracking-wide mb-2">{selectedAchievement.subTitle}</h2>
                <p className="text-zinc-400 text-sm font-semibold uppercase tracking-widest mb-4">{selectedAchievement.title}</p>
                
                <p className="text-zinc-300 text-sm leading-relaxed mb-6">{selectedAchievement.description}</p>

                <div className="space-y-3 bg-zinc-900/50 border border-zinc-800 rounded-xl p-5">
                  <div className="flex items-start gap-3">
                    <Calendar className="w-4 h-4 text-[#de4030] mt-0.5 flex-shrink-0" />
                    <div>
                      <p className="text-zinc-500 text-xs uppercase tracking-wider font-bold">Date</p>
                      <p className="text-white text-sm font-semibold">{selectedAchievement.date}</p>
                    </div>
                  </div>
                  <div className="flex items-start gap-3">
                    <MapPin className="w-4 h-4 text-[#de4030] mt-0.5 flex-shrink-0" />
                    <div>
                      <p className="text-zinc-500 text-xs uppercase tracking-wider font-bold">Venue</p>
                      <p className="text-white text-sm font-semibold">{selectedAchievement.venue}</p>
                    </div>
                  </div>
                  <div>
                    <p className="text-zinc-500 text-xs uppercase tracking-wider font-bold mb-1">Organizer</p>
                    <p className="text-white text-sm font-semibold">{selectedAchievement.organizer}</p>
                  </div>
                </div>
              </div>
            </motion.div>
